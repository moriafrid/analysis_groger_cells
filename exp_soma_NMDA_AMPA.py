...
for curr_i, model_place in tqdm(enumerate(folders)):
    ...
    for sec,seg in zip(secs,segs):
        ...
        V_spine=h.Vector()
        if isinstance(spine[0],float):
            V_spine.record(spine[1](spine[0])._ref_v)
        else:
            V_spine.record(spine[1](1)._ref_v)
        V_spines.append(V_spine)
        num+=1
    ...
    V_spine_All=[np.array(V)[cut_from_start_time:] for V in V_spines]
    ...
    V_spine_AMPA = [np.array(V)[cut_from_start_time:] for V in V_spines]
    V_spine_NMDA = [V_spine_All[i]-V_spine_AMPA[i] for i in range(len(V_spine_All))]
    # V_spine_NMDA = [V_spine_NMDA[i]+V_spine_All[i][0] for i in range(len(V_spine_All))]
    ...
    pickle.dump({'time':time_all,'voltage':{'Model':V_soma_All,'V_AMPA':V_soma_AMPA,'V_NMDA': V_NMDA+V_soma_All[0],'V_syn_AMPA':V_spine_AMPA,'V_syn_NMDA': V_spine_NMDA,'experiment':np.array(V_base)+loader.get_param('e_pas')},'parameters':    {'reletive_strengths':reletive_strengths,'PSD':psd_sizes,'RA':loader.get_param('Ra'),'RM':1.0/loader.get_param('g_pas'),'CM':loader.get_param('cm'),'E_PAS':loader.get_param('e_pas')}
