...
all_V_spine_base_NMDA,all_V_spine_head_NMDA,all_V_NMDA_NMDA,color_NMDA,all_W_AMPA_NMDA,all_W_NMDA_NMDA,all_V_soma_NMDA_NMDA=[],[],[],[],[],[],[]
...
fig2.subplots_adjust(left=0.25,right=0.95,top=0.95,bottom=0.15,hspace=0.2, wspace=0.25)
shapes = (1, 1)
...
plot_dict={'label':cell_name,'lw':scatter_size-1}
...
adgust_subplot(ax0_1,'AMPA-PSD' ,'PSD [um^2]','g_max AMPA [nS]',latter='A')
adgust_subplot(ax0_2,'NMDA-PSD','PSD [um^2]','g NMDA [nS]',latter='B')
adgust_subplot(ax0_3,'AMPA-NMDA','gmax AMPA [nS]','Vmax NMDA [mV]',latter='C')
all_AMPA,all_std_AMPA,all_NMDA,all_std_NMDA,all_PSD,all_AMPA_NMDA,all_PSD_NMDA,all_W_NMDA,all_V_NMDA,all_color=[],[],[],[],[],[],[],[],[],[]
for i,cell_name in enumerate(read_from_pickle('cells_name2.p')):
    print(cell_name)
    if cell_name=='2017_04_03_B':continue
...
add_curve_fit(ax0_1,all_PSD, all_AMPA,'g_density',name_units='pS/um^2')
add_curve_fit(ax0_2,all_PSD_NMDA, all_W_NMDA,'g_density',name_units='pS/um^2')
add_curve_fit(ax0_3,all_AMPA_NMDA, all_V_NMDA,'AMPA/NMDA',name_units='AU',x_m=0.62)
