...
if __name__=='__main__':
    ...
    all_W_AMPA_cumulativ,all_PSD_cumulativ,all_g_NMDA_cumulativ,N_cumulativ=[],[],[],[]
    N=np.arange(len(all_g_NMDA))+1
    ...
    for i,cell_name in enumerate(read_from_pickle('cells_name2.p')):
        ...
        while len(W_AMPA)==0:
            ...
            PSD=get_MOO_result_parameters(cell_name,'PSD',**dictMOO)
            distance=get_MOO_result_parameters(cell_name,'lambda',**dictMOO)
            j+=1
            print(W_AMPA)
    ...
    plt.savefig(save_dir+'/lambda_against_PSD_AMPA_NMDA_RA_min_error.png')
    plt.savefig(save_dir+'/lambda_against_PSD_AMPA_NMDA_RA_min_error.svg')
    # plt.show()
