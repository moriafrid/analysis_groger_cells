...
if __name__=='__main__':
    ...
    fig.subplots_adjust(left=0.05,right=0.95,top=0.9,bottom=0.1,hspace=0.05, wspace=0.1)
    ...
    ax1_3.text(-0.1, 1, 'D', transform=ax1_3.transAxes, size=22,weight="bold")
    ...
    for cell_name in read_from_pickle('cells_name2.p'):
        ...
        plot_morph(eval('ax1_'+str(i)), cell_name, before_after,without_axons=True,from_picture=from_picture,bbox_to_anchor=bbox_to_anchor)
        plot_syn_model(eval('ax2_'+str(i)),glob(base_dir+'AMPA&NMDA_soma_pickles_*'+decided_passive_params+'.p')[0],show_legend=False)
        i+=1
