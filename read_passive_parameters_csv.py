import pandas as pd
def get_passive_parameter(cell_name,double_spine_area=False,shrinkage_resize=[1.0,1.0],spine_start=20,fit_condition='const_param',file_type='z_correct.swc'):
    ...
    if double_spine_area:
        curr = df.loc[(df["fit_condition"] == fit_condition) & (df["file_type"] == file_type)&(df["spine_start"] == spine_start)&
              (df['shrinkage&resize_factors']==str(shrinkage_resize))&df['double_spine_area']==eval(double_spine_area), :]
    else:
        curr = df.loc[(df["fit_condition"] == fit_condition) & (df["file_type"] == file_type)&(df["spine_start"] == spine_start)&
