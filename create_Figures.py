...
def plot_syn_model2(ax,dirr):
    ...
    for k,color,label in zip(['voltage_all','voltage_0','voltage_1'],['black','#03d7fc','#fcba03'],['model','syn_0','syn1']):
        if k== 'voltage_all':
            ...
            label_AMPA='AMPA '+str(round(AMPA_weight*1000*reletive_strengths[num]/sum(reletive_strengths),3))+'nS '+str(round(reletive_strengths[num]*100))+"%"
            label_NMDA='NMDA '+str(round(NMDA_weight*1000*reletive_strengths[num]/sum(reletive_strengths),3))+'nS '+str(round(reletive_strengths[num]*100))+"%"
...
def plot_syn_voltage(ax,dirr):
    ...
    plt.legend(loc="upper left",prop={'size': label_size})
    x_to_ms=lambda x: x
    ax.add_artist(AnchoredHScaleBar(size_x=25, size_y=6, frameon=False, ax=ax, x_to_ms=x_to_ms,
...
if __name__=='__main__':
    for cell_name in read_from_pickle('cells_name2.p'):
        ...
        if get_n_spinese(cell_name)>1:
            ...
            plot_short_pulse_model(ax2,glob(base_dir+decided_passive_params+'_results.p')[0])
            plot_syn_model2(ax3,glob(base_dir+'AMPA&NMDA_soma_seperete_data_full_relative_'+decided_passive_params+'.p')[0])
            ...
            plot_syn_voltage(ax5,glob(base_dir+'Voltage Spine&Soma_data_full_relative_'+decided_passive_params+'.p')[0])
            ...
            plot_syn_model(ax5,glob(base_dir+'AMPA&NMDA_soma_data_full_same_'+decided_passive_params+'.p')[0])
